...
import sys
def ti():
    return datetime.dateast_time.now().timestamp()
...
def sites(A):
    ...
    for url in U:
        ...
        PS = r.text
        pen(str(PS),'recent_wall_sups.txt')
        if '502 Bad Gateway' in str(PS) or 'Server Error' in str(PS):
            return None
        JS = json.loads(PS)
    return JS
def keys():
...
def supply(add,Ad_pa):
    key = keys()
    CONT_SUP = 'http://api.snowtrace.io/api?module=account&action=tokenbalance&contractaddress='+str(add).lower()+'&address='+str(Ad_pa).lower()+'&tag=latest&apikey='+key
    return  CONT_SUP
def wall_sup(Ad_pa,B_L,B_G):
    key = keys()
    WALL_TR = 'http://api.snowtrace.io/api?module=account&action=tokentx&address='+str(Ad_pa)+'&startblock='+str(B_L)+'&endblock='+str(B_G)+'&sort=asc&apikey='+key
    return WALL_TR
def tok_sup(Ad_pa):
    key = keys()
    TOK_LP = 'https://api-cn.snowtrace.io/api?module=stats&action=tokensupply&contractaddress=' + str(Ad_pa).lower() + '&apikey='+key
    return TOK_LP
def JS_prep(J):
    import gc
    gc.collect()
...
def leaveit():
    ...
    #print(add)
    now = time.time()
    curr = int(now) - int(prev)
    if int(curr) > int(1000):
        js = get_txns(add,B_L,B_G)
    else:
        wait = (float(1)/float(int(1000) - int(curr)))+float(.1)
    ...
    while I != L_T:
        ...
        if tr != -1:
            ...
            if str(cont).lower() == str(adds).lower() or str(cont).lower() == str(adds).lower():
                val = float(W['value'])
                symbol = W['tokenSymbol']
                dec = W['tokenDecimal']
                if str(cont).lower() == str(adds).lower():
                    expo = float(10)**(float(-1)*float(dec))
                    bal = float(float(val) * float(expo))
                    if str(to).lower() == str(add).lower() and str(fro).lower() != str(add).lower():
                        tok_A_T = float(tok_A_T) + float(bal)
                    elif str(fro).lower() == str(add).lower() and str(to).lower() != str(add).lower():
                        tok_A_T = float(tok_A_T) - float(bal)
                    elif str(fro).lower() == str(add).lower() and str(to).lower() == str(add).lower():
                        tok_A_T = float(tok_A_T)
                    #rint(val_A)
...
def reques_timer():
    ...
    if float(i) < float(0.25):
        return (float(0.25) - float(i)), now
    return 0, now
def fold_check(x):
    try:
        mkdirs(x)
...
home_glob(home)
print(home)
varis = reader('variables.txt')
varis = json.loads(str(varis).replace("'",'"'))
print(home)
nodes = reader('nodes.txt')
nodes = json.loads(str(nodes).replace("'",'"'))
nodes = nodes["address"]
num = int(countit(str(nodes),',')) + int(1)
i= int(0)
varis = str(reader('variables.txt')).replace("'",'"')[:-1]+'}'
print(varis)
...
while i != num:
    node_new = nodes[i]
    print(node_new)
    i_sl,now = reques_timer()
    time.sleep(i_sl)
    link = supply(str(cont_add),str(node_new))
    JS = sites(link)
    ...
    if JS == None:
        ...
        js = JS["result"]
        pen(str(js),'recent_wall_sup.txt')
    dec = float(float(js) * float(expo))
    curr = (float(datetime.datetime.now().timestamp()))
    pen(str(curr),'last.txt')
    if float(dec) > float(0.00):
        could_tot_strong = float(tot_strong) + float(dec)
        if float(float(could_tot_strong) - float(tot_strong)) > float(0):
            tot_walls = int(tot_walls) + 1
            tot_strong = float(tot_strong) + float(dec)
        print('total_'+str(name)+':',tot_strong,',total_wallets: ',tot_walls,' ,out of: ',i,' wallets and ',num,' wallets total')
    i = i + 1
done = 'total_'+str(name)+':'+str(tot_strong)+',total_wallets: '+str(tot_walls)+' ,out of: '+str(i)+' wallets and '+str(num)+' wallets total'
pen(str(done),'wall_sups.txt')
    #print(i,num)
