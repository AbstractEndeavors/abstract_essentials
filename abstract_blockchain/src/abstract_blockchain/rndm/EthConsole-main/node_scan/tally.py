    ...
    while int(i) < int(300):
        ...
        while str(i) in js:
            ...
            if int(new) == int(i):
                ...
                js = str(str(bef_js).replace(']',',') + str(aft_js).replace('[','')).replace(' ','').replace('[,','[').replace("'",'"')
                pen(str(js).replace(',]',']').replace("'",'"'),'recent.txt')
                js = json.loads(str(js).replace(',]',']').replace("'",'"'))
                add_js = str(str(bef_add_js).replace(']',',') + str(aft_add_js).replace('[','')).replace(' ','').replace('[,','[').replace("'",'"')
                add_js = json.loads(add_js)
                #print(js)
...
home_glob(home)
print(home)
varis = reader(home+'\\'+'variables.txt')
varis = json.loads(str(varis).replace("'",'"'))
name = varis["name"]
print(home)
num_track = '{'  
tally = str(reader('nodes.txt')).replace("'",'"').replace('","','",\n"').replace(',,',',').replace(',]',']')
print(tally)
tally = json.loads(tally)
pen(str(tally),'nodes.txt')
nodes = tally['nodes']
adds = tally['address']
print(adds[1])
i = 0
...
js = nodes
tot_count = 0
num_wall = countit(adds,',')
js,num_track = countem(js,num_track,num_wall,adds)
def nogo():
    while i != num:
        js,num_track = countem(js,num_track,num_wall)
        num = countit(js,',')
        i = 0
        i = i + 1
    num_track = str(num_track+'}').replace(',}','}')
    i_count = 0
    i = 0
    num = countit(num_track,'"]')
    old = '{'
    while i != num:
        num_track = str(num_track).replace(',}','}').replace("'",'"')
        print(num_track)
        num_track = json.loads(num_track)
        if str(i) in num_track:
            done = '0'
            i_2 = int(0)
            while i_2 != num and done != '1':
                print(num_track[i_2])
                new_str = str(num_track[i_2]).split(':')
                new_str = str(new_str[0]).replace('"','')
                if int(i) == int(new_str):
                    old = str(old) +',\n' +str(num_track[str(new_str)])
                    done = '1'
                else:
                    i_2 = i_2 + int(1)
        i = i + 1
        print(old)
pen(str(num_track+'}').replace(',}','}'),'nums.txt')
num_track = str(num_track).replace(',}','}')
pen(str(num_track),'num_recent.txt')
num_track = json.loads(num_track)
i = 0
num = countit(num_track,'},')
print(num)
...
while int(i) != int(num):
    try:
        new = num_track[str(i)]
        perc = float(perc) + float(str(num_track[str(i)]["perc"]).replace('%',''))
        tot = str(tot) + '"'+str(i)+'"{"walls":"'+str(num_track[str(i)]["walls"])+'","perc":"'+str(num_track[str(i)]["perc"])+'","of":"'+str(perc)+'%"}\n'
        i = i + 1
    except:
        i = i + 1
pen(str(tot),str(name)+'_num_quick.txt')
