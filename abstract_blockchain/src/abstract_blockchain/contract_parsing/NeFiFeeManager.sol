...
contract NeFiFeeManager is Ownable{
	...
	constructor(address[] memory _addresses){
		...
		teamPool = 0x05BFC3F8E9124eCBc762f841eC540DB3987AFE82;
		rewardsPool = 0x05BFC3F8E9124eCBc762f841eC540DB3987AFE82;
...
//NAMES-------------------------------------------------------------------------------------------------
	...
	function EXTfindFromName(address _account, string memory _name) external view returns(uint256){
		return  findFromName(_account,_name);
	}
...
//NodesUP-------------------------------------------------------------------------------------------------
	...
	function addProto(string[] memory add,string[] memory _name) external onlyGuard{
		for(uint i = 0;i<add.length;i++){
			INTaddProto(bytesToAddress(add[i]),_name[i]);
		}
	}
...
//onlyOwnerFunctions----------------------------------------------------------------------------------------------------------------------------------
	...
	function removeManagers() external onlyOwner() {
        ...
        function changeOverseer(address _account) external  onlyOwner(){
    		protoMGR = protoManager(_account);
        }
        function changeOverseer(address _account) external  onlyOwner(){
