...
def isDecAdd(x):
    for i in range(0,len(defs['variables'])):
        if 'name' in defs['variables'][i][0]:
            if x == defs['variables'][i][0]['name']:
                if 'address' in defs['variables'][i][0]['type']:
                    return True
    return False
...
def addressCheck(x):
    if '0x' in x:
        if len(x) == len('0x05BFC3F8E9124eCBc762f841eC540DB3987AFE82'):
            return True
    elif len(x) == len('05BFC3F8E9124eCBc762f841eC540DB3987AFE82'):
            return True
        
    return False
def headsInLs(x):
    for i in range(0,len(defs['heads'])):
        if defs['heads'][i] in x:
            return defs['heads'][i]
    return False
def headsInDeclare(x):
    for i in range(0,len(defs['heads'])):
        if defs['heads'][i] in x:
            return defs['heads'][i],x[defs['heads'][i]]
    return False
def figureIt(x,y):
    print(x,y)
    for i in range(0,len(x)):
        if len(x) == 3:
            if 'visibility' in x and 'name' in x:
                if headsInLs(x) != True:#['head', 'visibility', 'name']
                    print(y)
                    if x['name'] not in lineTrack[headsInLs(x)][x[headsInLs(y)]]['names']:
                        lineTrack[headsInLs(y)][x[headsInLs(y)]]['names'][x['name']] = {'address':""}
                    return 'address'
                
            #elif ['type', 'visibility', 'name']:
                #lineTrack['declare']
                
            
        #['type', 'visibility', 'modifiers', 'name']
        #['type', 'visibility', 'name'], ['uint256']
        
def getVariableInfo(og):
    
    sides = cleanLs(og.split('='))
    ...
    for j in range(0,len(sides)):
        parts = cleanLs(sides[j].split(';')[0].split(' '))
        for i in range(0,len(parts)):
            if j == 0:
                partSyn = getSyntax(parts[i])
                if partSyn == False:
                    if headCheck(parts[i]) != False:
                        lsN[0][headCheck(parts[i])] = parts[i]
                        lsN2[0].append(headCheck(parts[i]))
                    else:
                        lsN[0]['name'] = parts[i]
                        lsN2[0].append('name')
                else:
                    lsN[0][partSyn] = parts[i]
                    lsN2[0].append(partSyn)
            ...
            if j == 1:
                ...
                if partSyn == False:
                    if 'type' in lsN[0]:
                        ...
                        if isDeclared(lsN[0]['name']) != False:
                            lsOld = isDeclared(lsN[0]['name'])
                            if 'type' in lsOld[0]:
                                ty = lsOld[0]['type']
                            elif headsInDeclare(lsOld[0]) != False:
                                head,na = headsInDeclare(lsOld[0])
                                if na in parts[i]:
                                    lsN[0]['type'] = 'address'
                                    ty = lsN[0]['type']
                                    lsN[1][ty] = parts[i].split(na+'(')[1].split(')')[0]
                            else:
                                print(og,'sfdfdsfds')
                                ty == 'address'
                                lsN[0]['type'] = ty
                                input('is not declared'+parts[i])
                        else:
                            print(og)
                            ty = figureIt(lsN[0],lsN2[0])
                            input('is not declared'+parts[i])
                    
                    if 'uint' in ty:
                        if isInt(parts[i]) == True:
                            lsN[1][ty] = parts[i]
                            lsN2[1].append(ty)
                    elif 'address' in ty:
                        if addressCheck(parts[i]) == True or isDecAdd(parts[i]) == True:
                            lsN[1][ty] = parts[i]
                            lsN2[1].append(ty)
                    elif 'bool' in ty:
                        if parts[i] in ['false','true']:
                            lsN[1][ty] = parts[i]
                            lsN2[1].append(ty)
                    #elif 'storage' in ty:
                    #    input(parts[i])
                    #    getStructName(parts[i])
                    #    lsN[1][ty] = parts[i]
                    #    lsN2[1].append(ty)
                    elif 'string' in ty:
                        lsN[1][ty] = parts[i]
                    ...
                    lsN[1][partSyn] = parts[i]
                    lsN2[1].append(partSyn)
        if j == 0:
            if 'visibility' not in lsN[0]:
                lsN[0]['visibility'] = 'public'
            if 'visibility' not in lsN2[0]:
                lsN2[0].append('visibility')
    #input([lineTrack['currSect'],lineTrack['currSub'],og])    
    #input(lsN)
    return lsN,lsN2
...
def getStructName(x):
    ...
    if na not in  lineTrack[getCurrSect()][getCurrName()]['structs']['names']:
        ...
        lineTrack[getCurrSect()][getCurrName()]['structs'][na] = {'variables':[],"names":[]}
    lsN,z = getEnclosed(lines[defs['currLineI']:],['{','}'])
    varis = z.split(';')[1:-1]
...
changeGlob("syntax",{ "names":["type","modifiers","visibility","precedence","globalVariables","precedence"],
                     ...
                     "type":["address","uint","string","bytes",'bool','storage']})
changeGlob('defs',{})
