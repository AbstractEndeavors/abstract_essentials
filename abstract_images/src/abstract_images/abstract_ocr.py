...
from abstract_utilities.compare_utils import  create_new_name,get_closest_match_from_list
...
def screenShot(screen_number=1, screen_resolution=(1920, 1080),file_path='screenshot.png'):
    # Calculate the bounding box for the specified screen
    x1 = (screen_number - 1) * screen_resolution[0]
    y1 = 0
    x2 = x1 + screen_resolution[0]
    y2 = screen_resolution[1]

    # Capture the specified screen

    ImageGrab.grab(bbox=(x1, y1, x2, y2)).save(file_path)
    return file_path
