...
def return_checks():
  ...
  # Add checkbox_layout to the main layout
  return [
...
def return_input_check():
    return [[[sg.T('suggested title: '),sg.Input(title,key='suggested_key'),sg.Checkbox('',default=True,key='check_key')],[sg.T('suggested revision: '),sg.Input(revision,key='suggested_value'),sg.Checkbox('',default=True,key='check_value')],[sg.T('commentary: '),sg.T(commentary)]],[sg.Button('Ok'), sg.Button('Cancel')]]
def create_win(win_st:str='window',title:str='',layout=[]):
    change_glob(win_st,sg.Window(title, layout, grab_anywhere=False, size=(800, 480), return_keyboard_events=True, finalize=True))
# Create the window
def win_revision(win:str='window'):
    win_st = 'window_revision'
    if get_event(get_globe(win)) == 'Check Prompt':
      response = check_prompt(get_value(get_globe(win))['new_key'],get_value(get_globe(win))['new_value'])
      ...
      if str(response['revision']).lower() == 'false':
          ...
          if title == 'none':
            title = get_value(get_globe(win))['new_key']
          if revision != 'none':
            create_win(win_st=win_st,title='revisions',layout=return_input_check())
            win_revision_ok(win_st)
            win_revision_save(win_st)
def win_revision_ok(win_st:str='window'):
    if get_event(get_globe(win_st)) == 'Ok':
        suggested = ['key','value']
        for k in range(0,len(suggested)):
            if get_value(get_globe(win_st))['check_'+suggested[k]] == True:
                window['new_'+suggested[k]].update(value='suggested_'+suggested[k])
def win_revision_save(win_st:str='window'):
    if get_event(get_globe(win_st)) == 'Save':
        # Update json_dict with current values
        # Save current state to json file
        pen(path_to_local_folder('entries.json'),json.dumps(add_js(return_prompts(),{get_event(get_value(win_st))['new_key']:get_value(get_globe(win_st))['new_value']})))
        sg.Popup('Saved!')
def win_checks(win_st='window'):
  while True:
      event, values = read_window(get_globe(win_st))
      if event == sg.WIN_CLOSED or event == 'Exit':  # if user closes window or clicks exit
          break
      win_revision(win=win_st)
