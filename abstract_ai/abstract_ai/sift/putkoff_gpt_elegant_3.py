...
def truncate_text(text, max_tokens):
    ...
    cut_positions = [truncated_text.rfind(s) for s in ['\n', 'def', '!', '.']]
    cut_position = max(cut_positions)
    if cut_position != -1:
        truncated_text = truncated_text[:cut_position + 1]
    else:
        # If no complete sentence is found, find the last complete word
        last_word_end = truncated_text.rfind(' ')
        # If a complete word is found, truncate up to its end
        if last_word_end != -1:
            truncated_text = truncated_text[:last_word_end]
    return truncated_text
def chunk_text(data, tokens_per_chunk):
    len_trun = 0
    chunks = []
    tokens_per_chunk = int(tokens_per_chunk)  # Convert to integer
    while len_trun < len(data):
        trunk = truncate_text(data[len_trun:], tokens_per_chunk)
        len_trun += len(trunk) + 1
        chunks.append(trunk)
    return chunks
def if_lasts(st,ls):
    for k in range(0,len(ls)):
        if st[-len(ls[k]):] == ls[k]:
            return len(ls[k])
def if_toks_count(z,splits,ls,j):
    lasts = if_lasts(z,ls)
    if count_tokens(str(z)) >= splits and lasts != None:
        return True,j-lasts
    
    return False,j+1
def fill_toks(data,toks):
    lsN,j = [],0
    for k in range(0,len(str(data))):
        string = str(data[j:k])
        if count_tokens(data[j:]) <= toks:
            lsN.append(data[j:])
            return lsN
        if count_tokens(string) >= toks:
            lsN.append(string)
            j = k+1
def find_toks(data,split):
    count = count_tokens(str(data))
    splits = count/split
    j,beg,z,ls = 0,0,'',[]
    for k in range(0,len(str(data))):
        z = z + data[j]
        bool_it,j = if_toks_count(z,splits,['\n', 'def', '!', '.'],j)
        if bool_it == True:
            ls.append(beg,j)
            beg,z = j,''
    return ls       
...
def get_prompt_data(window):
    event, values = window.read()
    data = get_content(values)
    win_update(window,'-DATA_TOTAL_TOKEN_COUNT-',count_tokens(data))
    return data
def get_prompt_all(window):
    event, values = window.read()
    pre,data = create_pre_prompt(window),get_prompt_data(window)
    prompt = pre+ data
    update_display(window,prompt)
    win_update(window,"-PROMPT_TOTAL_TOKEN_COUNT-",count_tokens(prompt))
    return prompt,pre,data
...
def if_over_float_zero(k):
    i = int(k)
    if float(i)-float(k) != float(0):
        return int(i)+1
    return int(i)
def update_tokens(win):
    event, values = win_read(win)
    prompt,pre,data=get_prompt_all(win)
    counts = {}
    counts['prompt'],counts['pre'],counts['data']=count_tokens(prompt),count_tokens(pre),count_tokens(data)
    max_tokens = get_default_tokens()
    desired_perc = values['-DESIRED_COMPLETION_SELECT-']
    desired_completion =float(float(max_tokens)*float(desired_perc))/float(100)
    win_update(win,'-DESIRED_COMPLETION_CALCULATION-',desired_completion)
    desired_input =max_tokens-desired_completion
    win_update(win,'-DESIRED_INPUT_CALCULATION-',desired_input)
    pre_prompt_token_count = counts['pre']
    alloted_input = desired_input-pre_prompt_token_count
    win_update(win,'-INPUT_TOKEN_ALLOTED-',alloted_input)
    DATA_token_count = counts['data']
    total_chunks = if_over_float_zero(DATA_token_count/alloted_input)
    win_update(win,'-TOTAL_CHUNKS-',total_chunks)
    tokens_per_chunk = DATA_token_count/total_chunks
    win_update(win,'-ALLOTED_PER_CHUNK-',tokens_per_chunk)
    lsN = fill_toks(str(get_content(values)),tokens_per_chunk)
    return lsN
def get_list_Nums(i,k):
    ...
    return ls
def get_pre_prompt():
    return [sg.Tab('pre-prompt',[[sg.T('Token Count:'),sg.Text(0,key="-RESPONSE_TOKEN_COUNT-"),sg.T('of '),sg.Text(get_default_tokens(),key="-MODEL_TOKEN_COUNT-"),
                                 ...
                                 [sg.Frame('Completion Percentage: ',[[sg.Combo(get_list_Nums(95,5),default_value=50,key='-DESIRED_INPUT_SELECT-')]])],
...
def get_pre_prompt():
    return [sg.Tab('pre-prompt',[[sg.Frame("Response Type:",[[sg.Combo(get_response_types(), default_value="instruction", key="-RESPONSE_TYPE-", enable_events=True)],])],
                                 ...
                                 [sg.Frame('Total DATA Token Count:',[[sg.Text(0,key="-DATA_TOTAL_TOKEN_COUNT-")]])],
                                 [sg.Frame('Total Prompt Token Count:',[[sg.Text(0,key="-PROMPT_TOTAL_TOKEN_COUNT-")]])],
                                 ...
                                 [sg.Frame('Completion Percentage: ',[[sg.Combo(get_list_Nums(95,5),default_value=50,key='-DESIRED_INPUT_SELECT-',enable_events=True)]])],
                                 ...
                                 [sg.Frame('Input Desired: ',[[sg.Text(get_default_tokens()*.5,key='-DESIRED_INPUT_CALCULATION-')]])],
                                 [sg.Frame('input default: ',[[sg.Text(get_default_tokens()-get_default_tokens()*.5,key='-INPUT_TOKEN_DEFAULT-')]])],
                                 [sg.Frame('input alloted: ',[[sg.Text(get_default_tokens()-get_default_tokens()*.5,key='-INPUT_TOKEN_ALLOTED-')]])],
                                 [sg.Frame('total chunks: ',[[sg.Text(get_default_tokens()-get_default_tokens()*.5,key='-TOTAL_CHUNKS-')]])],
                                 [sg.Frame('alloted per chunk: ',[[sg.Text(get_default_tokens()-get_default_tokens()*.5,key='-ALLOTED_PER_CHUNK-')]])],
                                 [sg.Multiline('', size=(None, None), key="-RESPONSE_DISPLAY-", autoscroll=True, expand_x=True, expand_y=True)]])]


def display_percs(window,typ,other_perc):
    event, values = window.read()
    max_tokens = win_update(window,'-MODEL_TOKEN_COUNT-',get_default_tokens())

    other_desired = max_tokens*float(other_perc)/float(100)
    other_alloted = other_desired
    if typ == "INPUT":
        
        
        other_alloted = other_desired-float(win_update(window,'-PRE_TOKEN_COUNT-',get_prompt_keys(window)['tokens'])-win_update(window,'-DATA_TOKEN_COUNT-',get_prompt_data(window)['tokens']))
    win_update(window,f'-{typ}_PERCENTAGE-',other_perc)
    win_update(window,f'-{typ}_ALLOTED-',other_alloted)
    win_update(window,f'-{typ}_DESIRED-',other_desired)
def get_file_select():
...
def get_gpt_layout():
    ...
    while True:
        ...
        if '_SELECT-' in event:
            ...
            win_update(window,f'-DESIRED_{opp_typ}_SELECT-',opp_perc)
            #win_update(window,f'-DESIRED_{typ}_SELECT-',perc)
            win_update(window,f'-DESIRED_{opp_typ}_CALCULATION-',float(float(max_tokens)*float(opp_perc))/float(100))
            #win_update(window,f'-DESIRED_{typ}_SELECT-',perc)
            #win_update(window,f'-DESIRED_{typ}_CALCULATION-',float(float(max_tokens)*float(perc))/float(100))
        ...
        if event == "-INPUT-":
            update_tokens(window)
        if event == "-IPUT-":
            prompt = get_prompt_all(window)# In your event loop:
            prompt_count = count_tokens(pro7mpt)
            ...
            desired_completion_perc =float(float(max_tokens)*float(values['-DESIRED_COMPLETION_SELECT-']))/float(100)
            desired_input_perc = 100-values['-DESIRED_COMPLETION_SELECT-']
            model_token_count = get_default_tokens()
            win_update(window,'-DESIRED_COMPLETION_CALCULATION-',float(model_token_count)*float(desired_completion_perc)/float(100))
            win_update(window,'-DESIRED_INPUT_CALCULATION-',float(model_token_count)*float(desired_input_perc)/float(100))
            update_tokens(window)
        ...
        if "GRAB" in event:
            if event == "-GRAB_URL-":
                ...
                if values["-FILE-"]:
                    window["-TAB_FILE_CONTENT-"].update(reader(values["-FILE-"]))
